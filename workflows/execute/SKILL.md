...
- **This workflow composes the atomic skills — that's the point.** It routes to `verification-before-completion` at the gate, to `diagnose` / `de-flaking-tests` when verification turns up trouble, and to `slop-cleanup` / `receiving-code-review` at the review pass. Each is a soft reference: use the named skill if it's installed; if not, apply the same discipline inline. The skills stay atomic and unaware of this sequence — the ordering lives here.

---

# Mode: autonomous-subagent

For a well-specified plan where you want continuous, isolated execution with cross-model review and minimal human-in-loop. Adapted from superpowers' `subagent-driven-development` (in the thank-you list — see CREDITS): a **fresh implementer subagent per task**, then **two distinct review stages**, looping until the plan is done.
...
2. **Implementer subagent.** Dispatch a fresh subagent with the task's full text and scene-setting context (adapt superpowers' `implementer-prompt`: implement exactly the task, write/keep tests, verify, **commit**, self-review, report status DONE / DONE_WITH_CONCERNS / BLOCKED / NEEDS_CONTEXT). Answer its questions before it proceeds; handle non-DONE statuses (more context, a more capable model, smaller pieces, or escalate to the human) rather than forcing a retry unchanged.
   *Alternative implementer:* the default is a Claude subagent, but for a well-specified task you can hand the coding to a different-family or cheaper executor via the **`delegate-coding`** skill (cursor-agent/codex/cheaper-claude in an isolated worktree behind a verify gate). Prefer it for **cross-family diversity** or **parallelism**, not as a blanket cost win (delegation is ~break-even on small tasks; it pays at large/generation-heavy ones). The two-stage controller review below still applies to whatever the implementer produces.
