...
| vet | [`vet`](vet/SKILL.md) | Deliberate cross-model review + gated fix loop on a finished change. Runs `scrutinize` (Claude's outsider pass) then `council` (cross-model attack, which adjudicates the two disagreement-first), **stops** for a decision, then hands fixes to `receiving-code-review` → `/simplify` / `slop-cleanup` and re-reviews until clean. |
